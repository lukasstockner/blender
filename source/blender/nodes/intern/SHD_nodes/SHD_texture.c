	...
	if(data && node->id) {
		...
		if(in[0]->hasinput) {
			...
			if(in[0]->datatype==NS_OSA_VECTORS) {
				float *fp= in[0]->data;
				retval= multitex_nodes((Tex *)node->id, vec, fp, fp+3, shi->geometry.osatex, &texres, thread, which_output, NULL, NULL);
			}
				...
				retval= multitex_nodes((Tex *)node->id, vec, dxt, dyt, shi->geometry.osatex, &texres, thread, which_output, NULL, NULL);
			}
			else
