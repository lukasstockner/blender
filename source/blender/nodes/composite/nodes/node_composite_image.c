...
static void cmp_node_image_add_multilayer_outputs(bNodeTree *ntree, bNode *node,
                                                  RenderLayer *rl, int is_rlayers,
                                                  LinkNodePair *available_sockets, int *prev_index)
{
	RenderPass *rpass;
	for (rpass = rl->passes.first; rpass; rpass = rpass->next) {
		...
		/* Special handling for Combined and Depth passes to ensure compatibility. */
		if (STREQ(rpass->name, RE_PASSNAME_COMBINED)) {
			cmp_node_image_add_pass_output(ntree, node, "Image", rpass->name, -1, type, is_rlayers, available_sockets, prev_index);
			cmp_node_image_add_pass_output(ntree, node, "Alpha", rpass->name, -1, SOCK_FLOAT, is_rlayers, available_sockets, prev_index);
		}
		else {
			cmp_node_image_add_pass_output(ntree, node, rpass->name, rpass->name, -1, type, is_rlayers, available_sockets, prev_index);
		}
	}
}
...
static void cmp_node_image_create_outputs(bNodeTree *ntree, bNode *node, LinkNodePair *available_sockets)
{
	...
	if (ima) {
		...
		if (ima->rr) {
			...
			if (rl) {
				cmp_node_image_add_multilayer_outputs(ntree, node, rl, false, available_sockets, &prev_index);
	...
	cmp_node_image_add_pass_output(ntree, node, "Image", RE_PASSNAME_COMBINED, RRES_OUT_IMAGE, -1, false, available_sockets, &prev_index);
	cmp_node_image_add_pass_output(ntree, node, "Alpha", RE_PASSNAME_COMBINED, RRES_OUT_ALPHA, -1, false, available_sockets, &prev_index);

	if (ima) {
		if (!ima->rr) {
			cmp_node_image_add_pass_output(ntree, node, RE_PASSNAME_Z, RE_PASSNAME_Z, RRES_OUT_Z, -1, false, available_sockets, &prev_index);
		}
		BKE_image_release_ibuf(ima, ibuf, NULL);
	}
}
...
static void cmp_node_rlayer_create_outputs(bNodeTree *ntree, bNode *node, LinkNodePair *available_sockets)
{
		...
		if (re) {
			...
			if (rr) {
				...
				if (srl) {
					...
					if (rl) {
						cmp_node_image_add_multilayer_outputs(ntree, node, rl, true, available_sockets, &prev_index);
						RE_ReleaseResult(re);
						return;
					}
				}
	...
	cmp_node_image_add_pass_output(ntree, node, "Image", RE_PASSNAME_COMBINED, RRES_OUT_IMAGE, -1, true, available_sockets, &prev_index);
	cmp_node_image_add_pass_output(ntree, node, "Alpha", RE_PASSNAME_COMBINED, RRES_OUT_ALPHA, -1, true, available_sockets, &prev_index);
}
