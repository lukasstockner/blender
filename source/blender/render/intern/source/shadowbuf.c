...
typedef struct ShadBuf {
	...
	struct ISBData *isb_result_transp[BLENDER_MAX_THREADS];
} ShadBuf;
...
static void make_jitter_weight_tab(Render *re, ShadBuf *shb, short filtertype) 
{
	...
	for(jit= shb->jit, a=0; a<tot; a++, jit+=2) {
		if(filtertype==LA_SHADBUF_TENT) 
			...
			shb->weight[a]= RE_filter_value(R_FILTER_GAUSS, 1.8f*sqrt(jit[0]*jit[0] + jit[1]*jit[1]), 1.6f);
		else
			shb->weight[a]= 1.0f;
...
{
	ObjectInstanceRen *obi= &re->db.objectinstance[obinr];
	Material *ma;

	if(strand) {
		StrandRen *strand= render_object_strand_get(obi->obr, facenr-1);
		ma= strand->buffer->ma;
	}
	else {
		VlakRen *vlr= render_object_vlak_get(obi->obr, (facenr-1) & RE_QUAD_MASK);
		ma= vlr->mat;
	}
...
static int compress_deepshadowbuf(Render *re, LampRen *lar, DeepTile *tile, RenderPart *pa, APixstr *apixbuf, APixstrand *apixbufstrand)
{
	...
	for(y=0; y<pa->recty; y++) {
		...
		for(x=0; x<pa->rectx; x++, ap++, aps++, offset++) {
			...
			if(tot == 0) {
				tile->deepbuf[offset]= NULL;
				tile->totbuf[offset]= 0;
				continue;
			}

			/* enlarge temporary buffers if necessary */
			if(tot*2 > dsbufsize) {
				if(dsbuf1) MEM_freeN(dsbuf1);
				if(dsbuf2) MEM_freeN(dsbuf2);

				while(dsbufsize < tot*2)
					dsbufsize += 1024;

				dsbuf1= MEM_mallocN(sizeof(DeepSample)*dsbufsize, "compress_deepbuf1");
				dsbuf2= MEM_mallocN(sizeof(DeepSample)*dsbufsize, "compress_deepbuf2");
			}

			/* fill samples */
			ds[0]= sampleds[0]= dsbuf1;
			for(c=1; c<totbuf; c++)
			...
			tile->deepbuf[offset]= dsbuf2;
			tile->totbuf[offset]= 0;

			/* merge buffers */
			dsb= tile->deepbuf[offset];
			while(1) {
				minz= 0;
				found= 0;
			...
			prevtot= tile->totbuf[offset];
			totsample += prevtot;

			/* do compression using slopes */
			newtot= compress_deepsamples(tile->deepbuf[offset], prevtot, shb->compressthresh);
			tile->totbuf[offset]= newtot;
			totsamplec += newtot;

			/* allocate final memory in arena */
			newbuf= BLI_memarena_alloc(tile->arena, sizeof(DeepSample)*newtot);
...
static void shadowbuf_autoclip(Render *re, LampRen *lar)
{
	...
	for(i=0, obi=re->db.instancetable.first; obi; i++, obi=obi->next) {
		...
		for(a=0; a< obr->totvert;a++) {
			if((a & 255)==0) ver= render_object_vert_get(obr, a);
			else ver++;
...
/************************* Shadow Buffer Rasterization ***********************/

typedef struct ShadBufTask {
	Render *re;
	LampRen *lar;
	RenderPart pa;
} ShadBufTask;

static void shadowbuf_task_add(ThreadQueue *queue, Render *re, LampRen *lar, RenderPart *pa)
{
	ShadBufTask *task= MEM_callocN(sizeof(ShadBufTask), "ShadBufTask");
	...
	BLI_thread_queue_push(queue, task);
}

static void flat_shadowbuf_rasterize(Render *re, LampRen *lar)
{
	ShadBuf *shb= lar->shb;
	int *rectz, samples;
	...
	for(samples=0; samples<shb->totbuf; samples++) {
		zbuffer_shadow(re, shb->persmat, lar, rectz, shb->size, lar->jitbuf[samples][0], lar->jitbuf[samples][1]);
		/* create Z tiles (for compression): this system is 24 bits!!! */
...
static void deep_shadowbuf_rasterize(Render *re, LampRen *lar, RenderPart *pa)
{
	APixstr *apixbuf;
	...
	tx= pa->disprect.xmin/shsample->tilex;
	ty= pa->disprect.ymin/shsample->tiley;
	tile= &shsample->deeptile[ty*shsample->xtiles + tx];

	tile->totbuf= MEM_callocN(sizeof(int)*shsample->tilex*shsample->tiley, "deeptotbuf");
	tile->deepbuf= MEM_callocN(sizeof(DeepSample*)*shsample->tilex*shsample->tiley, "deepbuf");
	tile->arena= BLI_memarena_new(BLI_MEMARENA_STD_BUFSIZE, "deeparena");

	/* zbuffering */
	apixbuf= MEM_callocN(sizeof(APixstr)*pa->rectx*pa->recty, "APixbuf");
	if(re->db.totstrand)
		apixbufstrand= MEM_callocN(sizeof(APixstrand)*pa->rectx*pa->recty, "APixbufstrand");
	
	zbuffer_abuf_shadow(re, pa, lar, shb->persmat, apixbuf, apixbufstrand,
		&apsmbase, shb->size, lar->buffers, (float(*)[2])lar->jitbuf);

	/* compress deep shadow buffer samples */
	if(compress_deepshadowbuf(re, lar, tile, pa, apixbuf, apixbufstrand) == 0) {
		/* free buffers if no samples in this tile */
		MEM_freeN(tile->totbuf);
		MEM_freeN(tile->deepbuf);
		BLI_memarena_free(tile->arena);
		tile->totbuf= NULL;
		tile->deepbuf= NULL;
		tile->arena= NULL;
	}
...
static void deep_shadowbuf_init(Render *re, LampRen *lar, ThreadQueue *queue)
{
	RenderPart *pa;
	ListBase parts;
	rcti rect;
	ShadBuf *shb= lar->shb;
	ShadSampleBuf *shsample;
	int partx, party, xparts, yparts;

	/* create shadow buffer memory */
	shsample= MEM_callocN(sizeof(ShadSampleBuf), "shad sample buf");
	BLI_addtail(&shb->buffers, shsample);
	...
	parts_list_create(&parts, &partx, &party, &rect, xparts, yparts, 0);
	for(pa=parts.first; pa; pa=pa->next)
		shadowbuf_task_add(queue, re, lar, pa);

	BLI_freelistN(&parts);

	/* create tiles */
	shsample->xtiles= xparts;
	shsample->ytiles= yparts;
	shsample->tilex= partx;
	shsample->tiley= party;
	shsample->deeptile= MEM_callocN(sizeof(DeepTile)*shsample->xtiles*shsample->ytiles, "deep tile");

	/* we compress multiple samples into one visibility function */
	shb->totbuf= 1;
}

static void shadowbuf_init(Render *re, LampRen *lar, ThreadQueue *queue)
{
	ShadBuf *shb= lar->shb;
	float wsize, twozero[2]= {0.0f, 0.0f}, angle, temp;
	...
	if(ELEM3(lar->buftype, LA_SHADBUF_REGULAR, LA_SHADBUF_HALFWAY, LA_SHADBUF_DEEP)) {
		...
		else if(shb->totbuf==9) lar->jitbuf= (float(*)[2])give_jitter_tab(3);
		else lar->jitbuf= (float(*)[2])twozero;
		BLI_unlock_thread(LOCK_CUSTOM1);

		/* zbuffering */
		if(lar->buftype == LA_SHADBUF_DEEP)
			deep_shadowbuf_init(re, lar, queue);
		else
			shadowbuf_task_add(queue, re, lar, NULL);
	}
}

/********************* Threaded Shadow Buffer Creation ***********************/
...
static void *do_shadowbuf_task_thread(void *queue)
{
	ShadBufTask *task;
	...
	while((task=BLI_thread_queue_pop(queue))) {
		if(!task->re->cb.test_break(task->re->cb.tbh)) {
			if(task->lar->buftype == LA_SHADBUF_DEEP)
				...
				flat_shadowbuf_rasterize(task->re, task->lar);
		}
...
void shadowbufs_make_threaded(Render *re)
{
	ThreadQueue *queue;
	ListBase threads;
	LampRen *lar;
	int a, totthread= 0, totshb= 0;
	int (*test_break)(void *);

	/* count shadow buffers */
	for(lar=re->db.lampren.first; lar; lar= lar->next)
		if(lar->shb)
			totshb++;
	...
	if(totthread <= 1) {
		...
		do_shadowbuf_task_thread(queue);
	}
	else {
		...
		do {
			...
			PIL_sleep_ms(50);
		} while(BLI_thread_queue_size(queue));
...
void shadowbuf_free(LampRen *lar)
{
	if(lar->shb) {
		ShadBuf *shb= lar->shb;
		ShadSampleBuf *shsample;
		...
		for(shsample= shb->buffers.first; shsample; shsample= shsample->next) {
			if(shsample->deeptile) {
				for(a=0; a<shsample->xtiles*shsample->ytiles; a++) {
					tile= &shsample->deeptile[a];

					if(tile->deepbuf) {
						v= shsample->tilex*shsample->tiley;
...
static float readdeepshadowbuf(ShadBuf *shb, ShadSampleBuf *shsample, int bias, int xs, int ys, int zs)
{
	...
	/* get tile */
	tx= xs/shsample->tilex;
	ty= ys/shsample->tiley;
	tile= &shsample->deeptile[ty*shsample->xtiles + tx];

	/* calc z */
	xs= xs%shsample->tilex;
	ys= ys%shsample->tiley;

	ofs= ys*shsample->tilex + xs;
	tot= (tile->totbuf)? tile->totbuf[ofs]: 0;
	if(tot == 0)
		return 1.0f;

	v= readdeepvisibility(tile->deepbuf[ofs], tot, zs, bias, &biast);

	if(biast != 0.0f) {
		/* in soft bias area */
		biasv= readdeepvisibility(tile->deepbuf[ofs], tot, zs, 0, 0);
...
float shadowbuf_test(Render *re, ShadBuf *shb, float *co, float *dxco, float *dyco, float inp, float mat_bias)
{
	ShadSampleBuf *shsample;
...
Gregory S. Johnson - William re-> Mark - Christopher A. Burns 
and
...
static void isb_bsp_fillfaces(Render *re, RenderPart *pa, LampRen *lar, ISBBranch *root)
{
	...
	zbuf_alloc_span(&zspan, size, size, re->cam.clipcrop);
	...
	for(i=0, obi=re->db.instancetable.first; obi; i++, obi=obi->next) {
		...
		for(a=0; a<obr->totvlak; a++) {
			...
			if(ok && (obi->lay & lay)) {
				...
				camera_matrix_co_to_hoco(winmat, hoco[0], vlr->v1->co); d1= testclip_minmax(hoco[0], minmaxf);
				camera_matrix_co_to_hoco(winmat, hoco[1], vlr->v2->co); d2= testclip_minmax(hoco[1], minmaxf);
				camera_matrix_co_to_hoco(winmat, hoco[2], vlr->v3->co); d3= testclip_minmax(hoco[2], minmaxf);
				if(vlr->v4) {
					camera_matrix_co_to_hoco(winmat, hoco[3], vlr->v4->co); d4= testclip_minmax(hoco[3], minmaxf);
				}
				...
				if(partclip==0) {
					...
					/* window clipping */
					c1= camera_hoco_test_clip(hoco[0]); 
					c2= camera_hoco_test_clip(hoco[1]); 
					c3= camera_hoco_test_clip(hoco[2]); 
					if(vlr->v4)
						c4= camera_hoco_test_clip(hoco[3]); 
...
static int viewpixel_to_lampbuf(Render *re, ShadBuf *shb, ObjectInstanceRen *obi, VlakRen *vlr, int quad, float x, float y, float *co)
{
	...
	if(re->cam.type == CAM_ORTHO) {
		...
		hoco[0]= (x - 0.5*re->cam.winx)*fx - re->cam.winmat[3][0]/re->cam.winmat[0][0];
		hoco[1]= (y - 0.5*re->cam.winy)*fy - re->cam.winmat[3][1]/re->cam.winmat[1][1];
...
static void isb_add_shadfac(Render *re, ISBShadfacA **isbsapp, MemArena *mem, int obi, int facenr, short shadfac, short samples)
{
	ISBShadfacA *new;
	float shadfacf;
...
static int isb_add_samples(Render *re, RenderPart *pa, ISBBranch *root, MemArena *memarena, ISBSample **samplebuf)
{
	int xi, yi, *xcos, *ycos;
...
static void isb_make_buffer(Render *re, RenderPart *pa, LampRen *lar)
{
	ShadBuf *shb= lar->shb;
	ISBData *isbdata;
	...
	if(root.box.xmin != (float)shb->size) {
		...
		bsp_err= isb_add_samples(re, pa, &root, memarena, samplebuf);
			...
			if(re->params.osa) {
				...
				for(rd= pa->rectdaps, x=pa->rectx*pa->recty; x>0; x--, rd++, isbsa++) {
					...
					if(*rd) {
						...
						while(ps) {
							if(ps->shadfac)
								isb_add_shadfac(re, isbsa, isbdata->memarena, ps->obi, ps->facenr, ps->shadfac, pxf_mask_count(&re->sample, ps->mask));
							ps= ps->next;
						}
					}
				}
...
static int isb_add_samples_transp(Render *re, RenderPart *pa, ISBBranch *root, MemArena *memarena, ISBSampleA ***samplebuf)
{
	int xi, yi, *xcos, *ycos;
	int sample, bsp_err= 0;
...
/* Ztransp version */
/* lar->shb, pa->rectz and pa->rectp should exist */
static void isb_make_buffer_transp(Render *re, RenderPart *pa, APixstr *apixbuf, LampRen *lar)
{
	ShadBuf *shb= lar->shb;
	ISBData *isbdata;
	...
	/* branches are added using memarena (32k branches) */
	memarena = BLI_memarena_new(0x8000 * sizeof(ISBBranch), "isb transp arena");
	BLI_memarena_use_calloc(memarena);
	...
	if(root.box.xmin != (float)shb->size) {
		...
		/* insert all samples in BSP now */
		bsp_err= isb_add_samples_transp(re, pa, &root, memarena, samplebuf);
...
void irregular_shadowbuf_create(Render *re, RenderPart *pa, APixstr *apixbuf)
{
	GroupObject *go;
	...
	for(go=re->db.lights.first; go; go= go->next) {
		...
		if(lar->type==LA_SPOT && lar->shb && lar->buftype==LA_SHADBUF_IRREGULAR) {
			...
			isb_make_buffer(re, pa, lar);
			
			if(apixbuf) {
				lar->shb->isb_result_transp[pa->thread]= MEM_callocN(sizeof(ISBData), "isb transp_data");
				isb_make_buffer_transp(re, pa, apixbuf, lar);
			}
		}
	}
}
...
static void isb_data_free(ISBData *isbdata)
{
	if(isbdata->shadfacs)
		MEM_freeN(isbdata->shadfacs);
	if(isbdata->shadfaca)
...
void irregular_shadowbuf_free(Render *re, RenderPart *pa)
{
	GroupObject *go;
	...
	for(go=re->db.lights.first; go; go= go->next) {
		...
		if(lar->type==LA_SPOT && lar->shb && lar->buftype==LA_SHADBUF_IRREGULAR) {
			...
			if(shb->isb_result[pa->thread]) {
				isb_data_free(shb->isb_result[pa->thread]);
				lar->shb->isb_result[pa->thread]= NULL;
			}

			if(shb->isb_result_transp[pa->thread]) {
				isb_data_free(shb->isb_result_transp[pa->thread]);
				lar->shb->isb_result_transp[pa->thread]= NULL;
			}
		}
	}
}

void shadowbuf_create(Render *re, LampRen *lar, float mat[][4])
{
	struct ShadBuf *shb;
	float viewinv[4][4], omat[4][4];
	...
	invert_m4_m4(viewinv, re->cam.viewmat);
...
void shadowbuf_rotate(LampRen *lar, float mat[][4], int restore)
{
	if(restore) {
		mul_m4_m4m4(lar->shb->persmat, lar->shb->viewmat, lar->shb->winmat);
	}
	else {
		float pmat[4][4], smat[4][4];
