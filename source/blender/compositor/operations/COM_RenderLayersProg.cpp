...
RenderLayersProg::RenderLayersProg(const char *passName, DataType type, int elementsize) : NodeOperation()
{
...
void RenderLayersProg::initExecution()
{
	...
	if (rr) {
		...
		if (srl) {
			...
			if (rl) {
				this->m_inputBuffer = RE_RenderLayerGetPass(rl, this->m_passName, this->m_viewName);
			}
		}
	}
	if (re) {
