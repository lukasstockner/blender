			...
			if (srl) {
				...
				if (rl) {
					for (int i = 0; i < numberOfOutputs; i++) {
						...
						RenderPass *rpass;
						for (rpass = (RenderPass*) rl->passes.first; rpass; rpass = rpass->next) {
							if (STREQ(storage->pass_name, rpass->name)) {
								break;
							}
						}
						if (rpass) {
							if (STREQ(rpass->name, RE_PASSNAME_COMBINED) && STREQ(output->getbNodeSocket()->name, "Alpha")) {
								...
								testSocketLink(converter, context, output, new RenderLayersAOOperation(rpass->name, COM_DT_VECTOR, rpass->channels), scene, layerId, false);
							}
							else {
