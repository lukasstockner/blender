...
// >>> XXX deprecated - old animation system
			...
			ob->dup_group_render = newlibadr_us(fd, ob->id.lib, ob->dup_group_render);
	...
	for (Scene *sce = main->scene.first; sce; sce = sce->id.next) {
		if (sce->id.tag & LIB_TAG_NEED_LINK) {
			...
			for (SceneRenderLayer *srl = sce->r.layers.first; srl; srl = srl->next) {
				...
				srl->world_override = newlibadr_us(fd, sce->id.lib, srl->world_override);
...
// XXX deprecated - old animation system (for version patching only)
	...
	if (ob->dup_group_render)
		expand_doit(fd, mainvar, ob->dup_group_render);
...
static void expand_scene(FileData *fd, Main *mainvar, Scene *sce)
{
	...
	for (srl = sce->r.layers.first; srl; srl = srl->next) {
		...
		expand_doit(fd, mainvar, srl->world_override);
