...
static int rna_Image_gl_load(Image *image, ReportList *reports, int filter, int mag)
{
	...
	if (!error) {
		...
		wrapmode = GL_CLAMP; // XXX jwilkins: CLAMP_TO_BORDER?
#else
		wrapmode = GL_CLAMP_TO_EDGE; //  not sure any textures in blender use the border so it shouldn't make a difference
#endif
		/* clean GL error buffer */
		while (glGetError() != GL_NO_ERROR) {}
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, image->tpageflag & IMA_CLAMP_U ? wrapmode : GL_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, image->tpageflag & IMA_CLAMP_V ? wrapmode : GL_REPEAT);
