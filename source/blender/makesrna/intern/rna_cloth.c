...
static void rna_def_cloth_sim_settings(BlenderRNA *brna)
{
	...
	prop= RNA_def_property(srna, "flexibility_damping", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "rigid_damp");
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_ui_text(prop, "Flexibility Damping", "Dampens velocities to be more like a stiff rigid body");
	RNA_def_property_update(prop, 0, "rna_cloth_update");
