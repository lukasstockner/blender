                                                ...
                                                int *global_track_index, bool draw_selected)
{
	...
	for (track = tracksbase->first; track; track = track->next) {
		...
		if (dflag & DRAW_PICKING)
			gpuSelectLoad(base->selcol + (tracknr << 16));
...
static DMDrawOption draw_dm_bweights__setDrawOptions(BMEditMesh *em, int index)
{
...
static void draw_dm_bweights__mapFunc(BMEditMesh *em, int index, const float co[3],
                                      const float UNUSED(no_f[3]), const short UNUSED(no_s[3]))
{
...
static void draw_dm_bweights(BMEditMesh *em, Scene *scene, DerivedMesh *dm)
{
	...
	if (ts->selectmode & SCE_SELECT_VERTEX) {
		gpuSpriteSize(UI_GetThemeValuef(TH_VERTEX_SIZE) + 2);
		gpuImmediateFormat_C4_V3();
		gpuBeginSprites();
...
/* 3. */
	switch (draw_as) {
		case PART_DRAW_DOT:
			if (part->draw_size)
				gpuSpriteSize(part->draw_size);
			else
				gpuSpriteSize(2.0);  /* default dot size */
			break;
		case PART_DRAW_CIRC:
...
/* 6. */
	...
	if (draw_as == PART_DRAW_PATH) {
		...
		if (select) {
			...
			if (part->draw_size) {
				gpuSpriteSize(part->draw_size + 2);
			}
			else {
				gpuSpriteSize(4.0);
			}
                              ...
                              Object *ob, DerivedMesh *dm, int facecol)
{
	...
	if (facecol && check_ob_drawface_dot(scene, v3d, ob->dt)) {
		gpuSpriteSize(UI_GetThemeValuef(TH_FACEDOT_SIZE));
