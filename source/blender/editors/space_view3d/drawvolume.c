...
#undef FULL_ON_FIRE
	...
	while (1) {
		...
		if (numpoints > 2) {
			...
			if (use_fire) {
				if (GLEW_VERSION_1_4)
					glBlendFuncSeparate(GL_SRC_ALPHA, GL_ONE, GL_ONE, GL_ONE); /* non-default blend function */
				else
			...
			/* render smoke slice */
			if (GLEW_VERSION_1_4)
				glBlendFuncSeparate(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE, GL_ONE_MINUS_SRC_ALPHA); /* default? */
			else
		...
		for (y = floor(y0); y < res_max[1]; y += step_size)
			for (z = floor(z0); z < res_max[2]; z += step_size) {
				...
				if (heat[index] >= 0.01f) {
					...
					GPU_point_size(24.0f * heat[index]);
