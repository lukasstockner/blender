...
static void recalcData_objects(TransInfo *t)
{
	...
	if (t->obedit) {
		...
		if ((t->animtimer) && (t->context) && IS_AUTOKEY_ON(t->scene)) {
			...
			autokeyframe_pose_cb_func(t->context, t->scene, (View3D *)t->view, ob, t->mode, targetless_ik, true);
		}
		...
		for (i = 0; i < t->total; i++) {
			...
			if ((t->animtimer) && IS_AUTOKEY_ON(t->scene)) {
				animrecord_check_state(t->scene, &ob->id, t->animtimer);
				autokeyframe_ob_cb_func(t->context, t->scene, (View3D *)t->view, ob, t->mode, true);
			}
