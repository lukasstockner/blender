...
static void drawEdgeSlide(const struct bContext *C, TransInfo *t)
{
	...
	// XXX jwilkins: this problem here is that there is not a policy that guarantees what state OpenGL will be in when we get here and what we can mess with and what needs to be restored after changes.
			...
			gpuBegin(GL_LINES);
			if (curr_sv->v_a) {
				gpuVertex3fv(curr_sv->v_a->co);
				gpuVertex3fv(curr_sv->v_co_orig);
			}
			if (curr_sv->v_b) {
				gpuVertex3fv(curr_sv->v_b->co);
				gpuVertex3fv(curr_sv->v_co_orig);
			}
			gpuEnd();
...
static void drawVertSlide(const struct bContext *C, TransInfo *t)
{
	...
	// XXX jwilkins: this problem here is that there is not a policy that guarantees what state OpenGL will be in when we get here and what we can mess with and what needs to be restored after changes.
			...
			if (is_clamp) {
				sv = sld->sv;
				for (i = 0; i < sld->totsv; i++, sv++) {
					gpuVertex3fv(sv->co_orig_3d);
					gpuVertex3fv(sv->co_link_orig_3d[sv->co_link_curr]);
				}
			}
			else {
				sv = sld->sv;
