	...
	if (totnode) {
		...
		if (brush->sculpt_tool != SCULPT_TOOL_SMOOTH && brush->autosmooth_factor > 0) {
			if (brush->flag & BRUSH_INVERSE_SMOOTH_PRESSURE) {
				smooth(sd, ob, nodes, totnode, brush->autosmooth_factor*(1-ss->cache->pressure)*ss->cache->autosmooth_overlap);
			}
			else {
				smooth(sd, ob, nodes, totnode, brush->autosmooth_factor*ss->cache->autosmooth_overlap);
			}
		}
