...
static int edbm_do_deform_laplacian_vertex_exec(bContext *C, wmOperator *op)
{
	Object *obedit = CTX_data_edit_object(C);
	BMEditMesh *em = BKE_editmesh_from_object(obedit);
	int usex = true, usey = true, usez = true, preserve_volume = true;
	...
	for (i = 0; i < repeat; i++) {
		if (!EDBM_op_callf(em, op,
		                   "deform_laplacian_vert verts=%hv lambda_factor=%f lambda_border=%f use_x=%b use_y=%b use_z=%b preserve_volume=%b",
		                   BM_ELEM_SELECT, lambda_factor, lambda_border, usex, usey, usez, preserve_volume))
		{
			return OPERATOR_CANCELLED;
		}
...
void MESH_OT_vertices_deform_laplacian(wmOperatorType *ot)
{
	/* identifiers */
	ot->name = "Laplacian Deform Vertex";
	ot->description = "Laplacian deform of selected vertices";
	ot->idname = "MESH_OT_vertices_deform_laplacian";
	
	/* api callbacks */
	ot->exec = edbm_do_deform_laplacian_vertex_exec;
	ot->poll = ED_operator_editmesh;
	...
	RNA_def_int(ot->srna, "repeat", 1, 1, 200,
	            "Number of iterations to deform the mesh", "", 1, 200);
	RNA_def_float(ot->srna, "lambda_factor", 0.00005f, 0.0000001f, 1000.0f,
	              "Lambda factor", "", 0.0000001f, 1000.0f);
	RNA_def_float(ot->srna, "lambda_border", 0.00005f, 0.0000001f, 1000.0f,
	              "Lambda factor in border", "", 0.0000001f, 1000.0f);
