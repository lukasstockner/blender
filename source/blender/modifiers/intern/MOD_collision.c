...
static void freeData(ModifierData *md)
{
	...
	if (collmd) {
		...
		if (collmd->mloop) {
			MEM_freeN((void *)collmd->mloop);
			collmd->mloop = NULL;
		}

		if (collmd->looptri) {
			MEM_freeN((void *)collmd->looptri);
			collmd->looptri = NULL;
		}
	...
	if (dm) {
		...
		if (current_time > collmd->time_xnew) {
			...
			if (collmd->time_xnew == -1000) { /* first time */
				...
				collmd->looptri_num = dm->getNumLoopTri(dm);
				collmd->mloop = dm->dupLoopArray(dm);
				collmd->looptri = MEM_mallocN(sizeof(*collmd->looptri) * collmd->looptri_num, __func__);
				memcpy(collmd->looptri, dm->getLoopTriArray(dm), sizeof(*collmd->looptri) * collmd->looptri_num);

				/* create bounding box hierarchy */
				collmd->bvhtree = bvhtree_build_from_mvert(
				        collmd->x, collmd->mloop,
				        collmd->looptri, collmd->looptri_num,
				        ob->pd->pdef_sboft);
				...
				if (!collmd->bvhtree) {
					...
					bvhtree_update_from_mvert(
					        collmd->bvhtree,
					        collmd->current_x, collmd->current_xnew, collmd->mloop,
					        collmd->looptri, collmd->looptri_num,
					        true);
				}
