...
void BKE_psys_collision_neartest_cb(void *userdata, int index, const BVHTreeRay *ray, BVHTreeRayHit *hit)
{
	...
	const MLoop *mloop = col->md->mloop;
	const MLoopTri *lt = &col->md->looptri[index];
	MVert *x = col->md->x;
	MVert *v = col->md->current_v;
	float t = hit->dist/col->original_ray_length;
	int collision = 0;

	pce.x[0] = x[mloop[lt->tri[0]].v].co;
	pce.x[1] = x[mloop[lt->tri[1]].v].co;
	pce.x[2] = x[mloop[lt->tri[2]].v].co;

	pce.v[0] = v[mloop[lt->tri[0]].v].co;
	pce.v[1] = v[mloop[lt->tri[1]].v].co;
	pce.v[2] = v[mloop[lt->tri[2]].v].co;
