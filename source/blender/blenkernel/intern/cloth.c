	...
	if (verts && lt) {
		for (i = 0; i < cloth->looptri_num; i++, lt++) {
			...
			if (moving) {
				...
				copy_v3_v3(co_moving[0], verts[mloop[lt->tri[0]].v].tx);
				copy_v3_v3(co_moving[1], verts[mloop[lt->tri[1]].v].tx);
				copy_v3_v3(co_moving[2], verts[mloop[lt->tri[2]].v].tx);
...
void cloth_free_modifier(ClothModifierData *clmd )
{
	...
	if ( cloth ) {
		...
		if (cloth->mloop)
			MEM_freeN(cloth->mloop);
		if (cloth->looptri)
			MEM_freeN(cloth->looptri);
...
static void cloth_from_mesh(ClothModifierData *clmd, DerivedMesh *dm)
{
	...
	const unsigned int mloop_num = dm->getNumLoops(dm);
	...
	clmd->clothObject->mloop_num = mloop_num;
	clmd->clothObject->mloop = MEM_mallocN(sizeof(*mloop) * mloop_num, "clothLoop");
	if (clmd->clothObject->mloop == NULL) {
		cloth_free_modifier(clmd);
		modifier_setError(&(clmd->modifier), "Out of memory on allocating clmd->clothObject->mloop");
		printf("cloth_free_modifier clmd->clothObject->mloop\n");
		return;
	}
	memcpy(clmd->clothObject->mloop, mloop, sizeof(*mloop) * mloop_num);


	clmd->clothObject->looptri_num = looptri_num;
	clmd->clothObject->looptri = MEM_mallocN(sizeof(*looptri) * looptri_num, "clothLoopTris");
	if (clmd->clothObject->looptri == NULL) {
		cloth_free_modifier(clmd);
		modifier_setError(&(clmd->modifier), "Out of memory on allocating clmd->clothObject->looptri");
		printf("cloth_free_modifier clmd->clothObject->looptri\n");
		return;
	}
	memcpy(clmd->clothObject->looptri, looptri, sizeof(*looptri) * looptri_num);
