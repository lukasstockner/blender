...
void collision_move_object(CollisionModifierData *collmd, float step, float prevstep)
{
	...
	bvhtree_update_from_mvert(
	        ...
	        collmd->mloop, collmd->looptri, collmd->looptri_num, true);
}
        ...
        float epsilon)
{
	...
	tree = BLI_bvhtree_new(looptri_num, epsilon, 4, 26);

	/* fill tree */
	for (i = 0, lt = looptri; i < looptri_num; i++, lt++) {
		float co[3][3];

		copy_v3_v3(co[0], mvert[mloop[lt->tri[0]].v].co);
		copy_v3_v3(co[1], mvert[mloop[lt->tri[1]].v].co);
		copy_v3_v3(co[2], mvert[mloop[lt->tri[2]].v].co);
...
void bvhtree_update_from_mvert(
        BVHTree *bvhtree,
        const MVert *mvert, const MVert *mvert_moving, const MLoop *mloop,
        const MLoopTri *looptri, int looptri_num,
        bool moving)
{
	const MLoopTri *lt;
	...
	for (i = 0, lt = looptri; i < looptri_num; i++, lt++) {
		...
		if (moving) {
			...
			copy_v3_v3(co_moving[0], mvert_moving[mloop[lt->tri[0]].v].co);
			copy_v3_v3(co_moving[1], mvert_moving[mloop[lt->tri[1]].v].co);
			copy_v3_v3(co_moving[2], mvert_moving[mloop[lt->tri[2]].v].co);
                                 ...
                                 BVHTreeOverlap *overlap, CollPair *collpair, float UNUSED(dt))
{
	...
	const MLoop *mloop_a, *mloop_b;
	const MLoopTri *lt_a, *lt_b;
#ifdef WITH_BULLET
	...
	mloop_a = clmd->clothObject->mloop;
	lt_a = &clmd->clothObject->looptri[overlap->indexA];

	mloop_b = collmd->mloop;
	lt_b = &collmd->looptri[overlap->indexB];

	/* fill face_a */
	collpair->ap1 = mloop_a[lt_a->tri[0]].v;
	collpair->ap2 = mloop_a[lt_a->tri[1]].v;
	collpair->ap3 = mloop_a[lt_a->tri[2]].v;

	/* fill face_b */
	collpair->bp1 = mloop_b[lt_b->tri[0]].v;
	collpair->bp2 = mloop_b[lt_b->tri[1]].v;
	collpair->bp3 = mloop_b[lt_b->tri[2]].v;
        ...
        BVHTreeOverlap *overlap, float epsilon, CollPair *collpair, float UNUSED(dt))
{
	...
	mloop = collmd->mloop;
	lt = &collmd->looptri[overlap->indexB];
...
int cloth_points_objcollision(Object *ob, ClothModifierData *clmd, float step, float dt)
{
	...
	cloth_bvh = BLI_bvhtree_new(mvert_num, MAX2(clmd->coll_parms->epsilon, clmd->coll_parms->distance_repel), 4, 6);
	/* fill tree */
	for (i = 0; i < mvert_num; i++) {
		float co[6];
