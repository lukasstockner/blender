                               ...
                               const bool tiled)
{
  ...
  for (view_id = 0; view_id < 2; view_id++) {
    ...
    image_assign_ibuf(ima, ibuf, stereo3d ? view_id : index, 0);
...
bool BKE_image_fill_tile(
    struct Image *ima, ImageTile *tile, int width, int height, const float color[4], int gen_type)
{
  if (!ima || !tile || ima->source != IMA_SRC_TILED) {
    return false;
  }
