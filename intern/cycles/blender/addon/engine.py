...
def register_passes(engine, scene, srl):
    ...
    crypto_depth = int(min(16, crl.pass_crypto_depth) / 2)
    if crl.use_pass_crypto_object:
        for i in range(0, crypto_depth): engine.register_pass(scene, srl, "AOV uCryptoObject{:02d}".format(i), 4, "RGBA", 'COLOR')
    if crl.use_pass_crypto_material:
        for i in range(0, crypto_depth): engine.register_pass(scene, srl, "AOV uCryptoMaterial{:02d}".format(i), 4, "RGBA", 'COLOR')
