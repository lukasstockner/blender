                            ...
                            const int face_num, const int vert_num)
{
	...
	if(mesh->smooth[face_num]) {
		...
		/* TODO(sergey): Make it utility function of Mesh, deduplicating
		 * with compute_face_normal().
		 */
		const Mesh::Triangle tri = mesh->get_triangle(face_num);
		const float3 v0 = mesh->verts[tri.v[0]];
		const float3 v1 = mesh->verts[tri.v[1]];
		const float3 v2 = mesh->verts[tri.v[2]];
		const float3 norm = cross(v1 - v0, v2 - v0);
		const float normlen = len(norm);
		if(normlen == 0.0f) {
			vN = make_float3(1.0f, 0.0f, 0.0f);
		}
		vN = norm / normlen;
	}
