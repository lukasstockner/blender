	...
	if(subdivision) {
		...
		for(b_mesh.vertex_colors.begin(l); l != b_mesh.vertex_colors.end(); ++l) {
			...
			for(b_mesh.polygons.begin(p); p != b_mesh.polygons.end(); ++p) {
				...
				for(int i = 0; i < n; i++) {
					...
					*(cdata++) = color_float_to_byte(color_srgb_to_linear(color));
				}
			}
		}
	}
		...
		for(b_mesh.tessface_vertex_colors.begin(l); l != b_mesh.tessface_vertex_colors.end(); ++l) {
			...
			for(l->data.begin(c); c != l->data.end(); ++c, ++i) {
				...
				uchar4 colors[4];
				colors[0] = color_float_to_byte(color_srgb_to_linear(get_float3(c->color1())));
				colors[1] = color_float_to_byte(color_srgb_to_linear(get_float3(c->color2())));
				colors[2] = color_float_to_byte(color_srgb_to_linear(get_float3(c->color3())));
				if(nverts[i] == 4) {
					colors[3] = color_float_to_byte(color_srgb_to_linear(get_float3(c->color4())));
				}
