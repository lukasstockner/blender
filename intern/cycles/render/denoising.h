...
public:
	StandaloneDenoiser(string in_path, string out_path, Device *device)
	 : in_path(in_path), out_path(out_path), device(device),
	   buffer(device, "prefilter buffer", MEM_READ_ONLY)
	{
		views = false;
		prefilter = false;
		samples = 0;
...
protected:
	...
	bool parse_channel_name(string name, string &renderlayer, string &pass, string &channel);
