...
void Integrator::device_update(Device *device, DeviceScene *dscene, Scene *scene)
{
	...
	kintegrator->use_sobol_dithering = 0;
	if(use_dithered_sampling) {
		assert(RNG_DITHER_SIZE == sobol_dither_matrix_size());
		kintegrator->use_sobol_dithering = 1;

		float2 *dither_matrix = dscene->sobol_dither.alloc(RNG_DITHER_SIZE*RNG_DITHER_SIZE);
