...
NODE_DEFINE(ImageTextureNode)
{
	...
	SOCKET_STRING(color_space, "Color Space", ustring("none"));
...
void ImageTextureNode::compile(OSLCompiler& compiler)
{
	...
	if(is_float == -1) {
		if(builtin_data == NULL) {
			...
			type = image_manager->get_image_metadata(filename.string(), NULL, color_space, &colorspace_data, builtin_free_cache);
...
void PointDensityTextureNode::compile(SVMCompiler& compiler)
{
	...
	if(use_density || use_color) {
		if(slot == -1) {
			bool is_float;
			slot = image_manager->add_image(filename.string(), ustring("none"), builtin_data,
			                                false, 0,
			                                is_float, NULL,
			                                interpolation,
			                                EXTENSION_CLIP,
