...
NODE_DEFINE(IESLightNode)
{
	...
	SOCKET_STRING(filename, "Filename", ustring(""));
	SOCKET_STRING(ies, "IES File", ustring(""));
...
void IESLightNode::compile(SVMCompiler& compiler)
{
	...
	compiler.add_node(NODE_IES,
		compiler.encode_uchar4(
			compiler.stack_assign_if_linked(input("Strength")),
			compiler.stack_assign_if_linked(input("Vector")),
			compiler.stack_assign(output("Fac")),
			0),
		slot, __float_as_int(strength));
}
