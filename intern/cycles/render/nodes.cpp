...
void CombineXYZNode::compile(SVMCompiler& compiler)
{
	...
	compiler.add_node(NODE_COMBINE_RGB, x_in->stack_offset, 0, vector_out->stack_offset);

	compiler.stack_assign(y_in);
	compiler.add_node(NODE_COMBINE_RGB, y_in->stack_offset, 1, vector_out->stack_offset);

	compiler.stack_assign(z_in);
	compiler.add_node(NODE_COMBINE_RGB, z_in->stack_offset, 2, vector_out->stack_offset);
}
...
void SeparateXYZNode::compile(SVMCompiler& compiler)
{
	...
	compiler.add_node(NODE_SEPARATE_RGB, vector_in->stack_offset, 1, y_out->stack_offset);

	compiler.stack_assign(z_out);
	compiler.add_node(NODE_SEPARATE_RGB, vector_in->stack_offset, 2, z_out->stack_offset);
}
