...
#define FOR_PIXEL_WINDOW_SSE     pixel_buffer = GET_PIXEL_BUFFER(low.x, low.y); \
                                               ...
                                               int pass_stride)
{
	if(feature_mode == FEATURE_MODE_RENDER) {
		...
		features[6] = fabs(ccl_get_feature_sse(4));
		...
		features[0] = ccl_get_feature_sse(0);
		features[1] = ccl_get_feature_sse(1);
		features[2] = ccl_get_feature_sse(2);
		features[3] = ccl_get_feature_sse(3);
		features[4] = ccl_get_feature_sse(4);
		features[5] = ccl_get_feature_sse(5);
		features[6] = fabs(ccl_get_feature_sse(6));
		if(feature_mode == FEATURE_MODE_BAKING_ALBEDO) {
			features[7] = ccl_get_feature_sse(14);
			features[8] = ccl_get_feature_sse(15);
			features[9] = ccl_get_feature_sse(16);
		}
                                                     ...
                                                     int pass_stride)
{
	if(feature_mode == FEATURE_MODE_RENDER) {
		...
		scales[0] = mask(active_pixels,
		                 sqr(ccl_get_feature_sse(0) - mean[0]) +
		                 sqr(ccl_get_feature_sse(1) - mean[1]) +
		                 sqr(ccl_get_feature_sse(2) - mean[2]));
		scales[1] = mask(active_pixels,
		                 sqr(ccl_get_feature_sse(3) - mean[3]) +
		                 sqr(ccl_get_feature_sse(4) - mean[4]) +
		                 sqr(ccl_get_feature_sse(5) - mean[5]));
		scales[2] = mask(active_pixels, fabs(fabs(ccl_get_feature_sse(4)) - mean[6]));
		if(feature_mode == FEATURE_MODE_BAKING_ALBEDO) {
			scales[3] = mask(active_pixels,
			                 sqr(ccl_get_feature_sse(14) - mean[7]) +
			                 sqr(ccl_get_feature_sse(15) - mean[8]) +
			                 sqr(ccl_get_feature_sse(16) - mean[9]));
		}
	}
}
...
ccl_device_inline void filter_calculate_scale_sse(float4 *scale, int feature_mode)
{
	if(feature_mode == FEATURE_MODE_RENDER) {
		...
		if(feature_mode == FEATURE_MODE_BAKING_ALBEDO) {
			scale[7] = scale[8] = scale[9] = rcp(max(reduce_max(sqrt(scale[3])), make_float4(0.01f)));
		}
		scale[6] = rcp(max(reduce_max(scale[2]), make_float4(0.01f)));
		scale[3] = scale[4] = scale[5] = rcp(max(reduce_max(sqrt(scale[1])), make_float4(0.01f)));
		scale[0] = scale[1] = scale[2] = rcp(max(reduce_max(sqrt(scale[0])), make_float4(0.01f)));
	}
}
