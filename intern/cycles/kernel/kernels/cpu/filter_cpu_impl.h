...
#else
	kernel_filter_write_feature(sample, x, y, load_int4(buffer_params), from, buffer, to_pass, load_int4(prefilter_rect));
#endif
}
...
#else
	kernel_filter_nlm_calc_difference(dx, dy, weight_image, variance, difference_image, load_int4(rect), stride, channel_offset, frame_offset, a, k_2);
#endif
}
...
#else
	kernel_filter_nlm_update_output(dx, dy, difference_image, image, out_image, accum_image, load_int4(rect), channel_offset, stride, f);
#endif
}
...
#else
	kernel_filter_nlm_construct_gramian(dx, dy, t, difference_image, buffer, transform, rank, XtWX, XtWY, load_int4(rect), load_int4(filter_window), stride, f, pass_stride, frame_offset, feature_mode);
#endif
}
