...
ccl_device float3 calculate_principled_sheen_brdf(const ShaderClosure *bsdf,
...
ccl_device int bsdf_principled_sheen_setup(ShaderClosure *bsdf)
{
	...
	const float3 omega_in, float *pdf)
{
	...
	if(dot(N, omega_in) > 0.0f) {
		*pdf = fmaxf(dot(N, omega_in), 0.0f) * M_1_PI_F;
		return calculate_principled_sheen_brdf(sc, N, V, L, H, pdf);
	}
	else {
		*pdf = 0.0f;
		return make_float3(0.0f, 0.0f, 0.0f);
	...
	float3 *domega_in_dy, float *pdf)
{
	...
	if(dot(Ng, *omega_in) > 0) {
		...
		*eval = calculate_principled_sheen_brdf(sc, N, I, *omega_in, H, pdf);
