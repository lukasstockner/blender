...
ccl_device ShaderClosure *subsurface_scatter_pick_closure(ccl_addr_space KernelGlobals *kg, ShaderData *sd, float *probability)
{
...
ccl_device void subsurface_color_bump_blur(ccl_addr_space KernelGlobals *kg, ShaderData *out_sd, ShaderData *in_sd, int state_flag, float3 *eval, float3 *N)
{
...
ccl_device int subsurface_scatter_multi_step(ccl_addr_space KernelGlobals *kg, ShaderData *sd, ShaderData bssrdf_sd[BSSRDF_MAX_HITS],
...
ccl_device void subsurface_scatter_step(ccl_addr_space KernelGlobals *kg, ShaderData *sd,
	int state_flag, ShaderClosure *sc, uint *lcg_state, float disk_u, float disk_v, bool all)
{
	float3 eval = make_float3(0.0f, 0.0f, 0.0f);
	uint num_hits = 0;
